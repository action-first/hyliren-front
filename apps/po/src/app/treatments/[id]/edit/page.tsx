...
export default function EditProcedurePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function submit(status: ProcedureStatus) {
    ...
    try {
      ...
      const localIds = new Set(form.variants.filter(v => !v.isNew).map(v => v.id));
      ...
      for (const v of form.variants) {
        ...
        if (v.isNew) {
          await proceduresApi.addVariant(id, member.id, body);
        } else {
          await proceduresApi.updateVariant(id, v.id, member.id, body);
        }
      }
