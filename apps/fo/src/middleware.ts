...
export const config = {
   ...
   *
   * `images` / `videos` / `character` 는 public/ 하위 정적 자산 디렉토리 —
   * 누락 시 middleware 가 lang prefix 없는 path 로 인식해 `/{lang}/{dir}/...` 로
   * redirect → 404 → public 자산 깨짐 결함 발생 (마스코트 PNG/MP4 포함).
   */
  matcher: [
    '/((?!api|_next/static|_next/image|images|videos|character|favicon.ico|icon.svg|robots.txt|sitemap.xml|manifest.json).*)',
  ],
};
